...
class ProductJsonld implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setSku($sku)
    {
        ...
        if ((!preg_match("/^([a-zA-Z]+\\d+.*)$/", $sku))) {
            throw new \InvalidArgumentException("invalid value for $sku when calling ProductJsonld., must conform to the pattern /^([a-zA-Z]+\\d+.*)$/.");
        }
    ...
    public function setEan($ean)
    {
        ...
        if (!is_null($ean) && (!preg_match("/^([0-9]{8,13})$/", $ean))) {
            throw new \InvalidArgumentException("invalid value for $ean when calling ProductJsonld., must conform to the pattern /^([0-9]{8,13})$/.");
        }
